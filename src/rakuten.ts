...
(async () => {
  try {
    ...
    // 合計金額取得
    const price = await page.$eval('#asset_total_asset > span:nth-child(1) > nobr', item => {
      return item.textContent;
    });
    // 評価損益
    const diffPrice = await page.$eval('#asset_total_asset_diff > span:nth-child(1) > nobr > span', item => {
      return item.textContent;
    });
